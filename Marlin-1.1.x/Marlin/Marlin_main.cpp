...
#if ENABLED(TWINDRAGON)
  uint8_t inactive_extruder = 1; //Julia3G code change, to keep track of inactive nozzle for purging purposes
  float extruder_y_pos[EXTRUDERS] = {239,0};  //T0,T1
  float EP_X = 190; //X position of entry point
  float Cent_Y =115; //Center of the two tools
  float NML_X = 259; //Docking Position X
  float Rem_X = 240;  // Position of X used to remove tool
#endif
...
inline void gcode_M211() {
  ...
  inline void gcode_M218() {
    ...
    #if ENABLED(DUAL_X_CARRIAGE) || ENABLED(SWITCHING_NOZZLE) || ENABLED(TWINDRAGON)
    ...
    if (tmp_extruder >= EXTRUDERS)
      ...
      if (tmp_extruder != active_extruder) {
        ...
        set_destination_to_current();
          ...
          #if ENABLED(TWINDRAGON)
          current_position[Z_AXIS] -= hotend_offset[Z_AXIS][active_extruder] - hotend_offset[Z_AXIS][tmp_extruder];
          inactive_extruder = active_extruder; //log the old active extruder as the inactive extruder as it is changing now, for purging purposes Twin Dragin
          #endif
...
void process_next_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 998: //tool change doc. It removes the currently active nozzle, and takes the inactive nozzle
          // ALl measurements should have been hade without any tool offsets. Depending on the offset that gets applied during toolchange, movements will be made according to T0 T0
           do_blocking_move_to_xy(EP_X+hotend_offset[X_AXIS][active_extruder],Cent_Y+hotend_offset[Y_AXIS][active_extruder],7000);
           do_blocking_move_to_xy(NML_X+hotend_offset[X_AXIS][active_extruder],Cent_Y+hotend_offset[Y_AXIS][active_extruder],7000);
           do_blocking_move_to_xy(NML_X+hotend_offset[X_AXIS][active_extruder],extruder_y_pos[active_extruder]+hotend_offset[Y_AXIS][active_extruder],7000);
           do_blocking_move_to_xy(Rem_X+hotend_offset[X_AXIS][active_extruder],extruder_y_pos[active_extruder]+hotend_offset[Y_AXIS][active_extruder],7000);
            //take new extruder
           do_blocking_move_to_xy(Rem_X+hotend_offset[X_AXIS][active_extruder],extruder_y_pos[inactive_extruder]+hotend_offset[Y_AXIS][active_extruder],7000);
           do_blocking_move_to_xy(NML_X+hotend_offset[X_AXIS][active_extruder],extruder_y_pos[inactive_extruder]+hotend_offset[Y_AXIS][active_extruder],7000);
           do_blocking_move_to_xy(NML_X+hotend_offset[X_AXIS][active_extruder],Cent_Y+hotend_offset[Y_AXIS][active_extruder],7000);
           //do_blocking_move_to_xy(EP_X+hotend_offset[Y_AXIS][active_extruder],EP_Y+hotend_offset[Y_AXIS][active_extruder],7000);
