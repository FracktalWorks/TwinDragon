...
# The Twin Dragon Electronics - Implementation and Wirings

The following documentation contains the contents about the pin connections, firmware configurations of the BigTreeTech Octopus V1.1 which is assembled into the Twin Dragon IDEX model of 300x250x298 buildsize.

## Pin Configurations:

![Twin dragon drivers and jumpers](https://user-images.githubusercontent.com/80109965/228223237-de8f5c97-4aea-4d5b-8413-bbc4c1b8ab5a.jpg)

-	Initially, insert the jumpers only in the 2nd set of pins from the left in each of the motor driver pin communication protocol pin set, in order to enable the UART communication protocol. This will enable communication between the motor drivers and the Raspberry Pi.
-	Simultaneously, set the jumper to the 3rd column in the set of fan voltage pins. This determines that the fan voltage supply has been set to 24V.
-	In the probe voltage pin set, insert the jumper to the left most pins, in order to set the probe supply to 24V.

![Twin dragon connections](https://user-images.githubusercontent.com/80109965/228224046-4c77a324-bd52-4e9c-ae52-9bf7b249f827.jpg)

-	From the above image, it is determined that the motor driver TMC2209 has been inserted into the slots of every motor driver. The pin slots above the motor driver determines the motor pins. Insert the motor pins in its respective slots as named in the above image.
-	Insert the heater connections in the respective heater pins present in the bottom left slots. They are connected in the order of HE0 – EXT0 heater, HE1 – EXT1 heater, HE3 – bed heater.
-	Beside the heater pins are the fan control pins, where they are defined according to the order of the multiple extruder fans. They are attached in the order of print cooling fan, hot end fan and controller fan. Simultaneously these set of pins are attached in the order of EXT0 and EXT1 fans.
-	Beside the fan pins (right side), are the limit switch pins assembled into 4 of them. The Top-left and the bottom-left are the X0 and X1 limit switches respectively. The consecutive pins in the first row are the Y and Z limit switches (mentioned in white colour over the pins). 
-	The pins placed two places above the limit switch pin slots are the thermistor pins, to measure the temperature of each of the heater’s temperature. They are attached in the sequence from left to right, consisting of Bed thermistor, E0 thermistor and E1 thermistor (mentioned in blue colour over the pins).
-	The top right pin set are the BLTouch pins, where the Z_OUT jumper is connected, which works as the signal pin for the probe.

# The Twin Dragon Firmware
## Klipper Firmware
...
#### [multi_pin #] ####
This is used to create an internal pin alias that can modify multiple output pins each time the alias pin is set.
* **'pin:'** is used to mention the multiple pin names that duplicates the functionality to these mentioned pins.

For more information [click here for the official Klipper documentation of [multi_pin]](https://www.klipper3d.org/Config_Reference.html?h=multi#multi_pin)
